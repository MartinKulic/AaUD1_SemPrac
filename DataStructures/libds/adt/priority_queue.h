...
namespace ds::adt {
    ...
    void SortedExplicitSequencePriorityQueue<P, T>::push(P priority, T data)
    {
        ...
        {
            dataFrontu = &this->getSequence()->insertFirst().data_;
        }
        else if (priority >= this->getSequence()->accessLast()->data_.priority_)
        {
            dataFrontu = &this->getSequence()->insertLast().data_;
        }
        else
        {
            //amt::SinglyLS<PriorityQueueItem<P, T>>::BlockType* blokPred = this->getSequence()->findPreviousToBlockWithProperty([&](amt::SinglyLS<PriorityQueueItem<P, T>>::BlockType* blok) {return blok->data_.priority_; });
            auto* blokPred = this->getSequence()->findPreviousToBlockWithProperty([&](auto* blok) {return blok->data_.priority_ >= priority; });
            dataFrontu = &this->getSequence()->insertAfter(*blokPred).data_;
        }
